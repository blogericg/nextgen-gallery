...
class C_Ajax_Controller extends C_MVC_Controller
{
	...
	function index_action($return=FALSE)
	{
		...
		if (!is_numeric($zlib) && $zlib == 'On') $buffer_limit = 1;
		else if (is_numeric($zlib) && $zlib > 0) $buffer_limit = 1;
		while (ob_get_level() != $buffer_limit) ob_end_clean();
	...
	{
		if ($token === TRUE) $token = isset($_REQUEST['nonce']) ? $_REQUEST['nonce'] : FALSE;
