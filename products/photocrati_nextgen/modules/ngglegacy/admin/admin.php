...
class nggAdminPanel
{
    ...
    function add_menu()
    {
        ...
        $manage_gallery_hook = add_submenu_page(NGGFOLDER,
            __('Manage Galleries', 'nggallery'),
            __('Manage Galleries', 'nggallery'),
        ...
        add_submenu_page(NGGFOLDER,
            _n('Manage Albums', 'Albums', 1, 'nggallery'),
            _n('Manage Albums', 'Manage Albums', 1, 'nggallery'),
            'NextGEN Edit album',
        ...
        // Handle POST submission *before* content is rendered and sent to the browser
        add_action('load-' . $manage_gallery_hook, function() {
            include_once(dirname(__FILE__) . '/functions.php'); // admin functions
            include_once(dirname(__FILE__) . '/manage.php');    // nggallery_admin_manage_gallery
            $manageGallery = nggManageGallery::get_instance();
            $manageGallery->pre_process();
        });
    ...
    function show_menu() {
        ...
        switch($_GET['page']) {
            case "nggallery-manage-gallery":
                ...
                $ngg->manage_page = nggManageGallery::get_instance();
                $ngg->manage_page->process();
    ...
    function edit_current_screen($screen) {
        ...
        if (strpos($screen->id, 'ngg') !== FALSE || strpos($screen->id, 'nextgen') !== FALSE || strpos($screen->id, 'ngg') === 0)
            $screen->ngg = TRUE;
