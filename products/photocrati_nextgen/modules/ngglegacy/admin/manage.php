...
class nggManageGallery
{
    ...
    public function pre_process()
    {
        ...
        if (isset($_POST['nggpage']) && $_POST['nggpage'] === 'manage-galleries')
            $this->cdn_pre_process_galleries();

        if (isset($_POST['nggpage']) && $_POST['nggpage'] === 'manage-images')
            $this->cdn_pre_process_images();
    }

    public function cdn_pre_process_galleries()
    {
            ...
            switch($_POST['bulkaction']) {
                ...
                case 'set_watermark':
                    array_map(
                        function($id) {
                            return \ReactrIO\Background\Job::create(
                                sprintf(__("Watermarking images for gallery #%d", 'nextgen-gallery'), $id),
                                'cdn_watermark_gallery',
                                $id
                            )->save('cdn');
                        },
                        $_POST['doaction']
                ...
                case 'import_meta':
                    array_map(
                        function($id) {
                            return \ReactrIO\Background\Job::create(
                                sprintf(__("Importing image metadata for gallery #%d", 'nextgen-gallery'), $id),
                                'cdn_import_metadata_gallery',
                                $id
                            )->save('cdn');
                        },
                        $_POST['doaction']
                    );
                    self::$messages[] = __('Import metadata job(s) created for selected galleries.', 'nggallery');
                    return;
                    break;
                ...
                case 'publish_to_cdn':
                    array_map(
                        function($id) {
                            return \ReactrIO\Background\Job::create(
                                sprintf(__("Publishing images for gallery #%d", 'nextgen-gallery'), $id),
                                'cdn_publish_gallery',
                                $id
                            )->save('cdn');
                        },
                    ...
                    self::$messages[] = __('Publish job(s) created for selected galleries.', 'nggallery');
                    return;
                    break;
            }
        }
        ...
        if (isset($_POST['TB_bulkaction']) && isset($_POST['TB_ResizeImages']))
        {
            ...
            array_map(
                function($id) {
                    ...
                    $dataset = [
                        ...
                        'size'      => 'full',
                        'params'    => [
                            'width'  => intval($_POST['imgWidth']),
                            'height' => intval($_POST['imgHeight'])
                        ]
                    ];
                    return \ReactrIO\Background\Job::create(
            ...
            self::$messages[] = __('Resize job(s) created for selected galleries.', 'nggallery');
            return;
        }
        ...
        if (isset($_POST['TB_bulkaction']) && isset($_POST['TB_NewThumbnail']))
        {
            ...
            array_map(
                function($id) {
                    return \ReactrIO\Background\Job::create(
                        sprintf(__("Generating new thumbnails for gallery #%d", 'nextgen-gallery'), $id),
                        'cdn_generate_thumbnail_gallery',
                        $id
                    )->save('cdn');
                },
                explode(',', $_POST['TB_imagelist'])
            );
            self::$messages[] = __('Generate new thumbnail job(s) created for selected galleries.', 'nggallery');
            return;
        }
    }

    public function cdn_pre_process_images() {
        if (isset($_POST['bulkaction']) && isset($_POST['doaction']))
        {
            ...
            switch($_POST['bulkaction']) {
                ...
                case 'rotate_cw':
                    ...
                    self::$messages[] = __('Rotate image job(s) created for selected images.', 'nggallery');
                    return;
                    break;
                ...
                case 'set_watermark':
                    ...
                    self::$messages[] = __('Watermark image job(s) created for selected images.', 'nggallery');
                    return;
                    break;
                ...
                case 'import_meta':
                    ...
                    self::$messages[] = __('Import metadata job(s) created for selected images.', 'nggallery');
                    return;
                    break;
            }
        }
        ...
        if (isset($_POST['TB_bulkaction']) && isset($_POST['TB_ResizeImages']))
        {
            ...
            $settings = C_NextGen_Settings::get_instance();
            $settings->set('imgWidth',  $width);
            $settings->set('imgHeight', $height);
            $settings->save();
            ...
            array_map(
                function($id) use ($width, $height) {
                    ...
                    return \ReactrIO\Background\Job::create(
                        sprintf(__("Resizing image #%d", 'nggallery'), $id),
                        "cdn_resize_image",
                        $dataset
                    )->save('cdn');
                },
        ...
        if (isset($_POST['TB_bulkaction']) && isset($_POST['TB_NewThumbnail']))
        {
            ...
            self::$messages[] = __('Generate new thumbnail job(s) created for selected images.', 'nggallery');
            return;
        }
    ...
    /**
     * @param string $output
     * @param C_Image|stdClass $picture
     * @return string
     */
    function render_image_column_1($output = '', $picture)
    {
     ...
     * @return mixed
     */
    function render_image_column_2($output = '', $picture)
    {
        return $picture->pid;
    }
     ...
     * @return string
     */
    function render_image_column_3($output = '', $picture)
    {
     ...
     * @return string
     */
    function render_image_column_4($output = '', $picture)
    {
     ...
     * @return string
     */
    function render_image_column_5($output = '', $picture)
    {
