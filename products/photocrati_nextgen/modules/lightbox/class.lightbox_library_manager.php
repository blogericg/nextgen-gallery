...
class C_Lightbox_Library_Manager
{
    ...
    function register_defaults()
    {
        ...
        $simplelightbox->scripts  = array(
            ...
            'photocrati-lightbox#simplelightbox/nextgen_simple_lightbox_init.js'
        );
        ...
        $shutter->styles    = array(
            'photocrati-lightbox#shutter/shutter.css'
        );
        $shutter->scripts   = array(
            'photocrati-lightbox#shutter/shutter.js',
            'photocrati-lightbox#shutter/nextgen_shutter.js'
        );
        $shutter->values    = array(
            'nextgen_shutter_i18n' => array(
        ...
        $shutter2->styles   = array(
            'photocrati-lightbox#shutter_reloaded/shutter.css'
        );
        $shutter2->scripts  = array(
            'photocrati-lightbox#shutter_reloaded/shutter.js',
            'photocrati-lightbox#shutter_reloaded/nextgen_shutter_reloaded.js'
        );
        $shutter2->values   = array(
            'nextgen_shutter2_i18n' => array(
        ...
        $thickbox->styles   = array(
            'wordpress#thickbox'
        );
        $thickbox->scripts  = array(
            'photocrati-lightbox#thickbox/nextgen_thickbox_init.js',
            'wordpress#thickbox'
        );
        $thickbox->values   = array(
            'nextgen_thickbox_i18n' => array(
    ...
    function register($name, $properties)
    {
        ...
        if (!isset($object->scripts))   $object->scripts    = array();
        if (!isset($object->styles))    $object->styles     = array();
        if (!isset($object->values))    $object->values     = array();
    ...
    function get($name)
    {
        ...
        if (isset($this->_lightboxes[$name])) $retval = $this->_lightboxes[$name];
    ...
    function enqueue($lightbox=NULL)
    {
        ...
        $this->_add_script_data(
            'ngg_common', // TODO: Should this be ngg_lightbox_context instead?
            'nextgen_lightbox_settings',
            array(
                'static_path' => M_Static_Assets::get_static_url('{placeholder}', 'photocrati-lightbox'),
