...
class C_CDN_Move_Image_Job extends \ReactrIO\Background\Job
{
    function run()
    {
        ...
        // moving of the image will be performed by the cdn_move_image_final task.
        if (empty($data['size']))
        {
            ...
            $parent_job_id = \ReactrIO\Background\Job::create(
                sprintf(__("Moving image #%d to gallery #%d final stage", 'nggallery'), $id, $destination_id),
                'cdn_move_image_final',
            ...
            if ($cdn->is_offload_enabled())
            {
                foreach ($sizes as $size) {
                    \ReactrIO\Background\Job::create(
                        sprintf(__("Downloading image #%d sized %s to move to gallery #%d", 'nggallery'), $id, $size, $destination_id),
                        'cdn_move_image',
